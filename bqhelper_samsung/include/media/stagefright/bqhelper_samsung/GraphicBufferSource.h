...
#include <media/stagefright/bqhelper_samsung/ComponentWrapper.h>
 ...
 */
class GraphicBufferSource : public FrucFacade::FrucListener,
                            public FrucFacade::FrucCallbackInterface {
public:
    GraphicBufferSource();
    ...
    // Samsung FrucFacade listener interface (vendor ABI)
    // --------------------------------------------------
    // GraphicBufferSource in the Samsung OMX stack additionally
    // implements the FrucFacade listener/callback interfaces for frame
    // rate conversion (FRC). The methods are exported from
    // libstagefright_bufferqueue_helper_vendor.so and called by
    // libstagefright_omx_vendor.so and the Fruc engine.
    void onInputPrepare(FrucFacade::FrucFrame *frame) override;
    void onInputProcessed(const FrucFacade::FrucFrame *frame) override;
    void onOutputStorageRequired(FrucFacade::FrucFrame *frame) override;
    void onOutputReady(const FrucFacade::FrucFrame *frame) override;
