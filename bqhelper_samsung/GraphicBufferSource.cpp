...
#include <media/stagefright/bqhelper_samsung/GraphicBufferSource.h>
#include <media/stagefright/bqhelper_samsung/FrameDropper.h>
...
/*
 * Samsung FrucFacade listener interface.
 *
 * These methods feed Samsung's frame-rate-conversion (FRC) engine
 * (FrucFacade::FrucCodec in libFrucSSMLib.so). FRC is only active when
 * the OMX component/engine was initialized for it; the vanilla path
 * (regular camera recording, screenrecord) never invokes them. They
 * are implemented as no-ops with logging so that any unexpected FRC
 * activation becomes visible in logcat.
 *
 * Blob reference behavior:
 *   onInputProcessed: empty
 *   onError:          log-only
 */
void GraphicBufferSource::onInputPrepare(FrucFacade::FrucFrame *frame) {
    ALOGW("onInputPrepare(%p) called - FRC path not implemented", frame);
}

void GraphicBufferSource::onInputProcessed(const FrucFacade::FrucFrame *frame) {
    ALOGV("onInputProcessed(%p)", frame);
}

void GraphicBufferSource::onOutputStorageRequired(FrucFacade::FrucFrame *frame) {
    ALOGW("onOutputStorageRequired(%p) called - FRC path not implemented", frame);
}

void GraphicBufferSource::onOutputReady(const FrucFacade::FrucFrame *frame) {
    ALOGW("onOutputReady(%p) called - FRC path not implemented", frame);
}

void GraphicBufferSource::onError() {
    ALOGE("GraphicBufferSource::onError (FrucFacade)");
}
